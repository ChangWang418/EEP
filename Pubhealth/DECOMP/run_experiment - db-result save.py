...
PUBHEALTH_TEST = os.path.join(DATA_DIR, "test.tsv")

# 输出目录：仍放在当前脚本所在目录（CoT）
OUTPUT_DIR = BASE_DIR
...
def extract_final_label(text: str) -> str:
    if not isinstance(text, str):
        return "ERROR"
    # 优先匹配 Final label: XXX
    m = re.search(r'Final\s*label\s*[:：]\s*(TRUE|FALSE|MIXTURE|UNPROVEN)\b',
                  text, flags=re.IGNORECASE)
    if m:
        return m.group(1).upper()
    # 其次匹配最后一次出现的标签
    matches = re.findall(r'\b(TRUE|FALSE|MIXTURE|UNPROVEN)\b', text, flags=re.IGNORECASE)
    if matches:
...
def run():
    ...
    for i, (claim, evidence, label) in enumerate(data):
        ...
        print(f"{global_index + 1}: Predicted Label = {prediction}")
