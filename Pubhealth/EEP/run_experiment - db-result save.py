...
# Output path (set as sibling "results" directory)
OUTPUT_DIR = BASE_DIR
os.makedirs(OUTPUT_DIR, exist_ok=True)  # Automatically create the output directory if it doesn't exist
...
def run():
    ...
    # ✅ Check how many result CSV files already exist
    file_index = 0
    ...
    for i, (claim, evidence, label) in enumerate(data):
        ...
        # Create a new file every batch_size entries
        if i % batch_size == 0:
            if f:
                f.close()
