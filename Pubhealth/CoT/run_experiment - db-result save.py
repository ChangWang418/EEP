...
# ===== 环境变量与 OpenAI Key：从项目根目录 .env 读取 =====
from dotenv import load_dotenv
import openai

# 当前文件所在目录： .../EEP-CoT-Selfask/Pubhealth/CoT
...
# dev/test 文件
PUBHEALTH_DEV = os.path.join(DATA_DIR, "dev.tsv")
PUBHEALTH_TEST = os.path.join(DATA_DIR, "test.tsv")  # 如果需要 test，把上面的变量换这里
...
def extract_final_label(output_text: str) -> str:
    ...
    pattern = re.compile(
        r'(?:FINAL\s*LABEL|FINAL|LABEL|结论|最终结论|最终标签)?\s*[:：]?\s*\b(TRUE|FALSE|MIXTURE|UNPROVEN)\b',
        flags=re.IGNORECASE
    )
    last_match = None
...
def run():
    # 选择 dev 或 test
    pubhealth_tsv = PUBHEALTH_DEV  # 如需 test：改为 PUBHEALTH_TEST
